...
class parking_ctrl(object):
	...
	def __init__(self):
		...
		self.perp_distance_left = 4.5 #distance from obstacle to the left side
	...
	def spin(self):
		while not rospy.is_shutdown():
			...
			if self.flag == 0:
				self.go_straight()
				self.left_limit=self.y+self.perp_distance_left+0.955
			...
			if self.compute_space == 1:
				if self.find_x1_obs == 1:
					if self.perp_distance_right<=4:
						self.y1_obs = self.y - 0.955 - self.perp_distance_right + 0.1 #the sensed position is not the widest part of the vehicle
				...
				if self.find_x2_obs == 1:
					if self.perp_distance_right<4:
						...
						self.y2_obs = self.y - 0.955 - self.perp_distance_right + 0.3
						# print self.x2_obs
						# print self.y2_obs
						# print self.y
						# print self.perp_distance_right
						# print self.yd
						self.find_x2_obs = 0
						...
						dist = self.x2_obs - self.x1_obs
						# print dist
						if dist >= 7.5: #safe parking space
							self.compute_space = 0
						else:
							self.compute_space = 1
							self.find_x1_obs = 1
	...
	def check_collision(self):
		while self.check_collision_flag==1:
			...
			if max(self.yfl)>=self.left_limit-0.2:
				self.xfr = []
				self.yfr = []
				self.xfl = []
				self.yfl = []
